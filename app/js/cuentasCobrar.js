...
function imprimirCuentaCobrar11(id,div){
var cuerpo="";
if(document.getElementById(id))
{
    ...
    $.ajax({
        ...
        success: function(resp) {
           ...
           '<div class="nombreProforma">'+resp['CuentaC']['1']+' '+resp['CuentaC']['2']+'</div>'+
...
function imprimirCuentaCobrar(id,div){
...
if(document.getElementById(id))
{
    ...
    $.ajax({
        ...
        success: function(resp) {
           ...
           '<center><h5 style="text-align: left;" class="CuentasTitulo">Cliente: '+(resp['CuentaC']['1'])+'</h5></center>'+
          ...
          {if(resp['DetalleCuentaC'].length>0 && resp['DetalleCuentaC'][0]['descripcion']){
           ...
           '<center><h5 class="CuentasTitulo">Abonos</h5>';
...
function CuentasCobrarPorCliente(div){
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                for(i=0;i<resp.length;i++)
                {
                        ...
                        '<center><h5 style="text-align: left;" class="CuentasTitulo">Cliente: '+(resp[i]['nombre'])+'</h5></center>'+
                        '</div>';
                        ...
                        if(resp[i]['Abonos'].length>0 && resp[i]['Abonos'][0][0])
                        {
                            ...
                            total=0;
                                    for(c=0;c<resp[i]['Abonos'].length;c++){
                                        ...
                                        '<td class="CuentasClienteColumna">'+(resp[i]['Abonos'][c][6])+'</td>'+
...
function CuentasCobrarVencidas(div){
    ...
    id=1;
        $.ajax({
            ...
            success: function(resp) {
                    ...
                    cuerpo+='<table  class="CuentasTabla">';
                    cuerpo+='<tr class="FilaCuentasCliente">'+
                                '<th class="CuentasClienteColumna">Fecha</th>'+
                                '<th class="CuentasClienteColumna">Plazo</th>'+
                                '<th class="CuentasClienteColumna">Dias Transcurridos</th>'+
                                '<th class="CuentasClienteColumna">Comprobante</th>'+
                                '<th class="CuentasClienteColumna">Cliente</th>'+
                                '<th class="CuentasClienteColumna">Total Credito</th>'+
                                '<th class="CuentasClienteColumna">Abonado</th>'+
                                '<th class="CuentasClienteColumna">Saldo</th>'+
                                '</tr>';
                    ...
                    if(resp.length>0 && resp[0][0])
                    {
                            for(c=0;c<resp.length;c++){
                                ...
                                '<td class="CuentasClienteColumna">'+(resp[c][7]+'')+' '+(resp[c][8]+'')+'</td>'+
                        ...
                        alert('Usted no tiene cuentas por Cobrar Vencidas')
                    }
